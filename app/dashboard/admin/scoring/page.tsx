...
import LeadScoringTable from "./lead-scoring-table";
...
export default async function AdminScoringPage() {
  ...
  const tierCounts = {
    A: leads.filter((l) => (l.fundabilityScore ?? 0) >= 80).length,
    B: leads.filter((l) => (l.fundabilityScore ?? 0) >= 65 && (l.fundabilityScore ?? 0) < 80).length,
    C: leads.filter((l) => (l.fundabilityScore ?? 0) < 65).length,
  };
  ...
  return <LeadScoringTable leads={serializedLeads} tierCounts={tierCounts} />;
}
